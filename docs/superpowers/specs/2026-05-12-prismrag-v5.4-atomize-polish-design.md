...
> 文档日期：2026-05-12（R2 更新：2026-05-16）
> 版本：PrismRag v5.4
...
不引入新 MCP 工具，不改变任何工具的参数签名。共涉及 4 个源文件（server.py、label_resolver.py 新建、incremental.py、vault_loader.py）+ 2 个测试文件。
...
| 文件 | 改动内容 |
|------|---------|
| `prism_rag/mcp_server/server.py` | P1: 注入 vault 路径到 server instructions；P2: `explain_node` 描述动作导向 + `search_knowledge` 描述交叉引用 + 纯 ID soft hint + explain_node 智能错误回执；P3: `list_knowledge_nodes` 加 `body_preview`（50字）+ `max_results=20` 默认 |
...
**范围说明**：v5.4 只注入主 namespace 路径。多 namespace 动态路径注入（需要 `contextvars` 或 session-scoped config）标记为 v5.5 技术债，不在本版本范围内（见决策日志 D5）。

**验收**：Jei 冒烟测试 S1——新 session 第一轮调用 `read_note` 不出现路径推断错误。
...
> **注意**：在 `explain_node` 内加先置检查（prior check）是死代码——E8 的调用路径是 Jei→`search_knowledge`，prior check 拦截不到。正确的插入点是 `search_knowledge` 内部。
...
当 query **精确匹配** `^KNOW-\d+$`（纯 ID，不含其他文字）时，返回 soft hint 而非执行搜索。hint 文案按位数分支：
...
**Schema 说明**：hint 文案满足三要素：① 声明搜索未执行；② 指定目标工具名；③ ⚠️ + 强制性动词。

**关键区分**：
- 纯 ID（`KNOW-000008`）→ 明确工具误用，触发 hint
- 混合 query（`对比 KNOW-000008 与 database 的设计`）→ 合法语义搜索，不触发 hint

### （d）`explain_node` 错误回执智能分支（防乒乓死循环）

`explain_node` 不做 KNOW-ID 格式校验（职责边界不变）。但 `resolve_entry_points` 无法精确匹配时，错误回执区分两种情况：
...
打破 search→explain→search 乒乓死循环：含杂质的 ID 要求清洗后重试 explain_node（而非踢回 search_knowledge）。

**验收**：
- 单元测试：`search_knowledge("KNOW-000008")` 返回 ⚠️ hint，`results=[]`
- 单元测试：`search_knowledge("KNOW-12")` 返回「格式不完整」hint，`results=[]`
- 单元测试：`search_knowledge("对比 KNOW-000008 与 database")` 正常搜索，不触发 hint
- 单元测试：`explain_node("KNOW-000008 的架构")` 返回「含额外文本」错误，提示纯 ID 重试
- Jei 冒烟测试 S2：断言A（System 层拦截）+ 断言B（Jei 最终调用 explain_node）两层都 pass
...
**修改**：每个节点追加 `body_preview` 字段，取 content 前 **50 字符**（约 25–40 个中文字，足够辨识节点主题），始终返回，不加控制参数；同时加 `max_results=20` 默认分页上限。
...
results.append({
    ...
    "body_preview": data.get("content", "")[:50],  # 约 25–40 中文字
})
```

```python
# list_knowledge_nodes 签名加默认分页（不是新参数，是现有参数的默认值调整）
...
**验收**：
- 单元测试：每个节点有 `body_preview`，长度 ≤ 50
- 单元测试：默认不传 `max_results` 时返回 ≤ 20 条
- Jei 冒烟测试 S3：Jei 对所有 KNOW 节点做主题聚类，回复包含具体 body 内容依据

---

## P4 — KNOW 节点 label 使用 frontmatter title

**问题**：`VaultDocument.label` = `path.stem`（文件名去掉扩展名）。v5.3 `atomize_apply` 生成的 KNOW 文件名格式为 `{kid}-{slug}.md`（如 `KNOW-000043-fresh-per-call-decision.md`），导致图中 label = `"KNOW-000043-fresh-per-call-decision"`。在力导向图 10-15 字符截断下，所有节点都显示为 `KNOW-00000...`，图谱可用性为零。

**修改**：新建 `ingest/label_resolver.py`，实现三层 fallback（`title → clean_slug → stem`）。`incremental.py` 和 `vault_loader.py` 都调用此 resolver（两个 loader 都负责 KNOW 节点 ingest）。
...
# 判断用 frontmatter.get('knowledge_id')，不用 doc.kind
# （doc.kind 的生命周期在 incremental 路径中不确定）
...
**注意**：改动仅影响新 ingest 的节点。已入图节点的 label 需下一次 `prism-rag ingest --full` 才更新，无需专门迁移命令。P2 的 soft hint 通过 query 字符串匹配，P4 不影响 P2。

**验收**：
- 单元测试：ingest 含 `title: "测试标题"` frontmatter 的 KNOW 文件，label = "测试标题"
- 单元测试：ingest 无 `title` 但有 slug 的 KNOW 文件，label = clean_slug 结果
- 单元测试：ingest 无 `title` 且无 slug 的 KNOW 文件，label = 文件名 stem
...
- `reset_federated` autouse fixture — 每个测试前后重置全局 `_federated` 等状态
- `_make_fake_federated(tmp_path)` helper — 构建最小 FederatedGraph stub

**关键架构决策**：soft hint 逻辑必须插在 `_ensure_federated()` 之前（纯输入校验，无需图）。这决定了测试 fixture 分组：

| 测试组 | 需要 FederatedGraph | 理由 |
|------|------|------|
| P2 soft hint（KNOW-000008、KNOW-12） | **否** | 提前返回，不触达图加载 |
| P2 混合 query / explain_node 测试 | 是 | 需要图数据 |
| P3 `body_preview` / `max_results` | 是（节点含 content） | 遍历图 |
| P4 `resolve_knowledge_label` | **否** | 纯函数 |

**`_make_fake_federated` 扩展**：补全 `knowledge_id` 字段，节点已有 `content`（现有 helper 已满足），无需大改。
...
| `test_search_knowledge_hint_id_uppercased` | **否** | `search_knowledge("know-000008")` 触发 hint，且 hint 文案中 ID 为大写 `KNOW-000008` |
...
| `test_explain_node_mixed_input_error` | 是 | `explain_node("KNOW-000008 的架构")` 返回「含额外文本」错误，且消息中含 `explain_node(node='KNOW-000008')` |
| `test_list_knowledge_nodes_has_body_preview` | 是 | 每个节点有 `body_preview`，长度 ≤ 50 |
| `test_list_knowledge_nodes_preview_truncates_at_50` | 是 | content 51字符的节点，`body_preview` 恰好等于 50 |
| `test_list_knowledge_nodes_preview_empty_ok` | 是 | 无 content 节点的 `body_preview` 为 `""` 不报错 |
| `test_list_knowledge_nodes_max_results_default` | 是 | 默认返回 ≤ 20 条，`total` 字段反映真实总数 |
...
| `test_resolve_knowledge_label_fallback_stem` | **否** | `resolve_knowledge_label({}, "KNOW-000001")` = `"KNOW-000001"` |
...
- **D2：`body_preview` 固定 50 字符（非 100 字），加 `max_results=20` 默认分页** — 初稿 100 字源于 v5.3 P3 建议（200 字压缩版）。`list_knowledge_nodes` 无相关度排序，50 字已足够辨识主题（≈25–40 个中文字），更少 token 更可靠。`max_results=20` 防止大 vault 时 token 膨胀（20 节点 × 50 字 ≈ 1,000 token 增量可控）。不加 `include_summary` 控制参数——参数越少越好，始终返回避免 Jei 需要额外传参。
- **D3：P4 新建 `label_resolver.py`，incremental + vault_loader 都调用；判断用 `frontmatter.get('knowledge_id')` 而非 `doc.kind`** — 辩论发现两处设计都比初稿更优：① 两个 loader 都负责 KNOW 节点 ingest，若只改 incremental.py，vault_loader 路径的 label 不会更新；② v5.3 已为 vault_loader 路径定义完整的 `kind` 生命周期，但 incremental 路径是否同步保证不能仅凭设计文档确认；`frontmatter.get('knowledge_id')` 是对两条路径都成立的最稳健判断，不依赖 `kind` 的设置时机。fallback 链升级为三层（title → slug 清洗 → stem），slug 清洗在力导向图截断场景下是图谱可用性的 0/1 分界。`VaultDocument.label` 不改，影响面不变。
- **D4：Jei 验收测试 = 3 条冒烟测试（非全流程对话测试）** — PrismRag 的最终受益者是调用它的 LLM，代码正确但 LLM 选错工具等于未修复，必须有 LLM 行为验证。但全流程对话测试依赖 Gemini CLI session 和 Quota，不适合 CI。改为 3 条固定路径冒烟测试（S1/S2/S3），验收时手动执行，判定标准是工具选择正确与否。标记 `@pytest.mark.skip` 不进 CI。
- **D5：P1 多 namespace 动态 vault 路径注入推迟至 v5.5** — v5.4 只注入主 namespace 路径（单 vault 场景覆盖绝大多数实际用例）。多 namespace 场景需要 `contextvars` 或 session-scoped config 才能在请求级别动态注入正确路径，架构变化较大。v5.3 测试 E1 的失败发生在单 namespace 场景（路径完全缺失），v5.4 静态注入已修复。
- **D6：soft hint 逻辑必须插在 `_ensure_federated()` 之前** — `search_knowledge` 首行即调用 `_ensure_federated()` 加载整个图。soft hint 是纯输入校验，无需图数据。插在之前：① soft hint 测试无需 fake FederatedGraph fixture，极简；② 快速失败语义清晰；③ 避免为明显误用查询触发不必要的图加载。`label_resolver` 同理，纯函数直接单元测试，无需任何 fixture。 — v5.4 只注入主 namespace 路径（单 vault 场景覆盖绝大多数实际用例）。多 namespace 场景需要 `contextvars` 或 session-scoped config 才能在请求级别动态注入正确路径，架构变化较大。v5.3 测试 E1 的失败发生在单 namespace 场景（路径完全缺失），v5.4 静态注入已修复该场景。多 namespace 动态注入标记为 v5.5 技术债。
