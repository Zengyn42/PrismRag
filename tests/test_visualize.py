...
def test_federation_html_contains_all_namespaces(tmp_path):
    """Generated federation HTML must mention all namespace names as nodes."""
    from prism_rag.report.federation_map import generate_federation_html

    fg = _make_multi_graph(tmp_path)
    out = tmp_path / "federation.html"
    generate_federation_html(fg, out)

    html = out.read_text(encoding="utf-8")
    assert "nimbus" in html, "Namespace 'nimbus' not in federation HTML"
    assert "code" in html, "Namespace 'code' not in federation HTML"


def test_federation_html_has_portal_js(tmp_path):
    """Federation HTML must contain the federation portal JS."""
    from prism_rag.report.federation_map import generate_federation_html
    ...
    html = out.read_text(encoding="utf-8")
    assert "_federationPortals" in html, "Federation portal JS not injected"
    assert "window.open" in html, "window.open not in federation JS"


def test_federation_html_portal_map_valid(tmp_path):
    """_federationPortals JS map must be valid JSON with portal hrefs."""
    import re
    from prism_rag.report.federation_map import generate_federation_html
    ...
    html = out.read_text(encoding="utf-8")
    m = re.search(r'var _federationPortals = ({.*?});', html, re.DOTALL)
    assert m is not None, "_federationPortals assignment not found in federation HTML"
    portals = json.loads(m.group(1))
    assert "nimbus" in portals
    assert portals["nimbus"] == "nimbus/graph.html"
    assert "code" in portals
    assert portals["code"] == "code/graph.html"


def test_federation_html_node_size_info(tmp_path):
    """Federation node labels must include node/edge counts."""
    from prism_rag.report.federation_map import generate_federation_html
    ...
    html = out.read_text(encoding="utf-8")
    # Labels are like "nimbus\n1N / 0E"
    assert "1N" in html or "N /" in html, "Node count not in federation label"
